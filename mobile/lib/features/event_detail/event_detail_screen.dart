...
class _EventDetailScreenState extends State<EventDetailScreen>
  ...
  bool denyViewer() {
    ...
    showTopSnack(
      ...
      const SnackBar(
        content: Text(
          'Mode lihat saja — perlu peran OWNER/ADMIN untuk mengubah',
        ),
      ),
    );
    return true;
  ...
  void _onSync() async {
    final c = SyncEngine.instance.pendingByEvent[widget.event.id] ?? pending;
    final ids = await LocalDb.instance.outboxIds(widget.event.id);
    if (!mounted) return;
    if (c != pending ||
  ...
  Future<void> _refreshAll() async {
    ...
    conflicts = await SyncEngine.instance.conflictOps(widget.event.id);
    if (mounted) setState(() {});
  }
  ...
  Future<void> _pullServer() async {
    try {
      ...
      await LocalDb.instance.setMeta(
        'lastPull:${widget.event.id}',
        '${j['pulledAt'] ?? ''}',
      );
      // shortcuts server (best-effort, merge + jumlah untuk chips)
      try {
        final s = await dio.get(
          '/api/events/${widget.event.id}/guests/shortcuts',
        );
        final sj = Map<String, dynamic>.from(s.data as Map);
        final at = (sj['alamatTop'] as List? ?? []).cast<Map>().toList();
        final nt = (sj['nominalTop'] as List? ?? []).cast<Map>().toList();
        if (at.isNotEmpty) {
  ...
  Widget _buildSuggestOverlay(BuildContext ctx) {
    final box = _namaKey.currentContext?.findRenderObject() as RenderBox?;
  ...
  KeyEventResult _onNominalKey(FocusNode node, KeyEvent e) {
    ...
    if (e.logicalKey == LogicalKeyboardKey.arrowRight ||
      ...
      setState(
        () =>
            nominalHi = nominalHi < 0 ? 0 : (nominalHi + 1) % nominalTop.length,
      );
      ...
      if (mounted) {
        ...
        setState(
          () => liveDup = found == null
              ...
              : {
                  'nama': '${found['nama']}',
                  'nominalFormatted': formatRp((found['nominal'] as int?) ?? 0),
                  'source': 'lokal',
                },
        );
      }
  ...
  KeyEventResult _onNamaKey(FocusNode node, KeyEvent e) {
    ...
    if (e.logicalKey == LogicalKeyboardKey.arrowUp) {
      setState(
        () => suggestHi = (suggestHi - 1 + suggest.length) % suggest.length,
      );
      _refreshSuggestOverlay();
      return KeyEventResult.handled;
    }
    ...
    try {
      ...
      try {
        ...
        final r = await dio.post(
          '/api/events/${widget.event.id}/guests',
          data: {
            'nama': nama,
            'alamat': alamat,
            'nominal': nominal,
  ...
  List<Widget> get _tabActions => [
    ...
    IconButton(
      ...
      onPressed: () => Navigator.of(context)
          .push(
            MaterialPageRoute(
              builder: (_) => EventSettingsScreen(event: widget.event),
            ),
          )
          .then((v) {
      ...
      child: LayoutBuilder(
        builder: (context, cons) {
          ...
          return Scaffold(
            ...
            body: TabBarView(
              controller: tab,
              children: [_inputTab(), _booksTab(), _rekapTab()],
            ),
          );
        },
      ),
  ...
  Widget _wideScaffold() {
    return Scaffold(
      appBar: AppBar(
        title: Text(widget.event.namaAcara, overflow: TextOverflow.ellipsis),
        actions: _tabActions,
      ),
  ...
  Widget _nominalField() => Column(
    ...
    children: [
      Row(
        mainAxisAlignment: MainAxisAlignment.spaceBetween,
        children: [
          Text('Nominal *', style: Theme.of(context).textTheme.labelLarge),
        ...
        children: nominalTop.asMap().entries.map((e) {
          ...
          final chip = ActionChip(
            ...
            backgroundColor: active
                ...
                : null,
            side: BorderSide(
              color: active || hi ? scheme.primary : scheme.outlineVariant,
            ),
            label: Text.rich(
              TextSpan(
                children: [
      ...
      Wrap(
        ...
        children: methodes.map((m) {
          ...
          return ChoiceChip(
            ...
            onSelected: canEdit ? (_) => setState(() => metode = m) : null,
          );
        }).toList(),
      ),
    ],
  ...
  Widget _catatanField() {
    ...
    return TextField(
      ...
      decoration: InputDecoration(
        labelText: dup ? 'Catatan (wajib isi, duplikat)' : 'Catatan (opsional)',
        hintText: dup ? 'Wajib: bedakan dari data sebelumnya' : 'Opsional',
  ...
  List<Map<String, dynamic>> get shownGuests {
    ...
    int cmp(Map<String, dynamic> a, Map<String, dynamic> b) {
      int r;
      if (sortBy == 'Nama') {
        r = '${a['nama']}'.toLowerCase().compareTo(
          '${b['nama']}'.toLowerCase(),
        );
      } else if (sortBy == 'Nominal') {
        r = (((a['nominal'] as int?) ?? 0)).compareTo(
          ((b['nominal'] as int?) ?? 0),
        );
      } else {
          ...
          child: Row(
            children: [
              ...
              ChoiceChip(
                ...
                onSelected: (_) => setState(() => mejaFilter = null),
              ),
              ...mejaList.map(
                (m) => Padding(
                  ...
                  child: ChoiceChip(
                    ...
                    onSelected: (_) =>
                        setState(() => mejaFilter = mejaFilter == m ? null : m),
                  ),
                ),
              ),
              ...
              ChoiceChip(
                ...
                onSelected: (_) => setState(() => kasirFilter = null),
              ),
              ...kasirIds
                  .take(8)
                  .map(
                    (pid) => Padding(
                      ...
                      child: ChoiceChip(
                        ...
                        onSelected: (_) => setState(
                          () => kasirFilter = kasirFilter == pid ? null : pid,
                        ),
                      ),
                    ),
                  ),
                ...
                child: Wrap(
                  ...
                  children: [
                    ...
                    if (all.length > data.length)
                      TextButton.icon(
                        onPressed: () => setState(() => guestLimit += 20),
      ...
      rows: List.generate(data.length, (idx) {
        ...
        return DataRow(
          cells: [
            ...
            DataCell(
              Tooltip(
                message: '${_kasirName(g)} • ${g['petugasId'] ?? '—'}',
                child: ConstrainedBox(
            ...
            DataCell(MethodChip('${g['metode'] ?? 'AMPLOP'}')),
            DataCell(
              canEdit
                  ? IconButton(
                      tooltip: 'Aksi',
  ...
  Widget _inputTab() => LayoutBuilder(
    builder: (context, cons) {
          ...
          children: [
            if (goneServer)
              Card(
                ...
                child: ListTile(
                  ...
                  title: Text(
                    'Acara ini sudah dihapus dari server',
                    style: TextStyle(
                      fontWeight: FontWeight.w700,
                      color: Theme.of(context).colorScheme.onErrorContainer,
                    ),
            ...
            if (conflicts.isNotEmpty)
              Card(
                ...
                child: ListTile(
                  ...
                  title: Text(
                    '${conflicts.length} butuh catatan',
                    style: TextStyle(
                      fontWeight: FontWeight.w700,
                      color: Theme.of(context).colorScheme.onErrorContainer,
                    ),
                  ...
                  subtitle: Text(
                    'Duplikat nama+alamat — tambah penanda.',
                    style: TextStyle(
                      color: Theme.of(context).colorScheme.onErrorContainer,
                    ),
                  ),
                ...
                ...(medium
                    ...
                    : [
                        ...List.generate(
                          ...
                          (i) => _guestTile(
                            shownGuests.take(guestLimit).toList()[i],
                            i + 1,
                          ),
                        ),
                    ...
                    children: [
                      ...
                      if (medium)
                        Row(
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: [
                            Expanded(child: Column(children: [_namaField()])),
                            const SizedBox(width: 12),
                            Expanded(
                              child: Column(
                                children: [
            ...
            if (canEdit && !medium)
              Align(
                ...
                child: SizedBox(width: double.infinity, child: _saveButton()),
              ),
            if (canEdit) const SizedBox(height: 16),
            if (canEdit)
              Text(
                'Terakhir di perangkat ini (${shownGuests.length})',
                style: Theme.of(context).textTheme.titleSmall,
              ),
  ...
  Widget _bookTile(Map<String, dynamic> b, [int? no]) {
    ...
    return Card(
      ...
      child: ListTile(
        ...
        title: Text(nm, style: const TextStyle(fontWeight: FontWeight.w700)),
        subtitle: Text(
          '${b['alamat']}',
          maxLines: 2,
          overflow: TextOverflow.ellipsis,
            ...
            child: Wrap(
              ...
              children: [
                ...
                if (shown.length > data.length)
                  TextButton.icon(
                    onPressed: () => setState(() => bookLimit += 50),
                    icon: const Icon(Icons.expand_more_outlined, size: 16),
                    label: Text(
  ...
  Widget _booksTab() {
    final shown = bookQ.trim().isEmpty
        ...
        : books
              .where(
                (b) =>
                    '${b['nama']}'.toLowerCase().contains(
                      bookQ.toLowerCase(),
                    ) ||
                    '${b['alamat']}'.toLowerCase().contains(
                      bookQ.toLowerCase(),
                    ),
              )
              .toList();
            ...
            children: [
              ...
              Padding(
                ...
                child: TextField(
                  ...
                  decoration: InputDecoration(
                    hintText: 'Cari buku tamu… (${books.length}) (Ctrl+F)',
                    border: const OutlineInputBorder(),
                    filled: true,
                    isDense: true,
                    prefixIcon: const Icon(Icons.search_outlined),
    ...
    await showAdaptiveSheet(
      ...
      builder: (_) => SafeArea(
        child: ListView(
          ...
          children: [
            Text(
              'Butuh catatan (${conflicts.length})',
            ...
            ...conflicts.map((op) {
              ...
              return Card(
                ...
                child: ListTile(
                  ...
                  subtitle: Text('${v['nominal']} • ${v['metode']}'),
  ...
  Future<void> _resolveDialog(String opId, Map<String, String> v) async {
    final c = TextEditingController();
    ...
    if (ok == null) {
      ...
      await SyncEngine.instance.discardOp(widget.event.id, opId);
    ...
    if (c.text.trim().isEmpty) {
      if (mounted) {
        showTopSnack(
          context,
          const SnackBar(content: Text('Catatan wajib untuk bedakan duplikat')),
        );
      }
      return;
    }
    await SyncEngine.instance.resolveConflict(opId, c.text.trim());
    await SyncEngine.instance.flush(widget.event.id);
    await _refreshAll();
  }
  ...
  Future<void> _editGuestDialog(Map<String, dynamic> g) async {
    ...
    final nom = TextEditingController(text: '${g['nominal'] ?? ''}');
    ...
    if (!isValidNama(nama)) {
      if (mounted) {
        showTopSnack(
          context,
          const SnackBar(
            content: Text('Nama hanya boleh huruf, tanpa angka/simbol'),
          ),
        );
      }
      return;
  ...
  Future<void> _pushGuestEdit(String id, Map<String, dynamic> fields) async {
    if (denyViewer()) return;
    await LocalDb.instance.updateGuestLocal(id, fields);
    await LocalDb.instance.enqueue(widget.event.id, 'UPDATE_GUEST', 'guests', {
      'id': id,
      'fields': fields,
    });
    try {
      final dio = await ApiClient.instance.dio();
      final r = await dio.patch('/api/guests/$id', data: fields);
      if (r.statusCode == 200) {
        final ops = await LocalDb.instance.outboxList(widget.event.id);
        await LocalDb.instance.outboxRemove(
          ops
              .where(
                (o) => o['action'] == 'UPDATE_GUEST' && jsonPayloadId(o) == id,
              )
              .map((o) => '${o['id']}')
              .toList(),
        );
  ...
  Future<void> _deleteGuest(String id, String nama) async {
    ...
    await LocalDb.instance.enqueue(widget.event.id, 'DELETE_GUEST', 'guests', {
      'id': id,
    });
    try {
      final dio = await ApiClient.instance.dio();
      ...
      await LocalDb.instance.outboxRemove(
        ops
            .where(
              (o) => o['action'] == 'DELETE_GUEST' && jsonPayloadId(o) == id,
            )
            .map((o) => '${o['id']}')
            .toList(),
      );
        ...
        content: Column(
          ...
          children: [
            TextField(
              controller: n,
              textCapitalization: TextCapitalization.words,
              decoration: const InputDecoration(labelText: 'Nama (huruf saja)'),
            ),
            TextField(
              controller: a,
              textCapitalization: TextCapitalization.words,
              ...
              child: Padding(
                ...
                child: Column(
                  children: [
                    ...
                    Text(
                      formatRp(t['totalNominal'] as int),
  ...
  Widget _rekapAlamatTile(Map e) => Card(
    ...
    child: ListTile(
      ...
      trailing: Text('${e['jumlah']} • ${formatRp(e['total'] as int)}'),
    ),
  );
    ...
    await showWideDialog(
      ...
      builder: (ctx) => StatefulBuilder(
        builder: (ctx, setS) => AlertDialog(
            ...
            child: Column(
              ...
              children: [
                ...
                SegmentedButton<String>(
                  segments: const [
                    ButtonSegment(value: 'pemberian', label: Text('Pemberian')),
                    ButtonSegment(value: 'tamu', label: Text('Buku Tamu')),
                  ],
                  selected: {opt.type},
                ...
                const SizedBox(height: 12),
                Text('Urutan', style: Theme.of(context).textTheme.labelLarge),
                const SizedBox(height: 6),
                DropdownButtonFormField<String>(
                  initialValue: opt.order,
                  items: exportOrderLabels().entries
                      .map(
                        (e) => DropdownMenuItem(
                          value: e.key,
                          child: Text(
          ...
          actions: [
            TextButton(
              onPressed: busy.isNotEmpty ? null : () => Navigator.pop(ctx),
  ...
  Future<void> _runExport(ExportOptions opt, bool pdf) async {
    try {
      ...
      if (mounted) {
        showTopSnack(context, SnackBar(content: Text('Export gagal: $e')));
      }
    }
  }
}
