...
class LocalDb {
  ...
  Future<void> removeSyncedIds({
    ...
    if (guestIds.isNotEmpty) {
      final p = List.filled(guestIds.length, '?').join(',');
      await d.delete('guests', where: 'id IN ($p)', whereArgs: guestIds);
    }
    if (bookIds.isNotEmpty) {
      final p = List.filled(bookIds.length, '?').join(',');
      await d.delete('guest_books', where: 'id IN ($p)', whereArgs: bookIds);
    }
  }
  ...
  Future<int> dedupeSyncedLocalGuests(String eventId) async {
    ...
    for (final group in byKey.values) {
      ...
      final hasServerTwin = group.any((r) => !'${r['id']}'.startsWith('gst-'));
